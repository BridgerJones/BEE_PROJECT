...
def get_file_paths(dataset, divsion, shadow=False):
    ...
    if shadow:
        shadowbees = glob.glob(basepath + dataset + "/" + divsion + '/shadow_bee/*/*.png')
        for file in shadowbees:
            file_paths.append((file, SHADOW_BEE))
    file_paths = shuffle(file_paths)
    return file_paths

def image_process(file_paths, gray_scale=False, normalize=True, to_categorical=True, flatten=False):
    """
